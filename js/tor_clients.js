...
var tc_change;
var $list_tcs = $( "#list-tcs" );
var $tc_prop = $( "#tc-prop" );

$list_tcs.on( "change", function () {
	var val = $list_tcs.find( ":selected" ).attr( "data" );
	val = val.split( '|' );
	$( "#TC_comment" ).val( val[ 0 ] );
	$( "#TC_client" ).find( "[value=" + val[ 1 ] + "]" ).prop( "selected", "selected" );
...
if ( $list_tcs.find( "option" ).length > 0 ) {
	$list_tcs.find( ":nth-child(1)" ).prop( "selected", "selected" ).change();
} else {
	$tc_prop.find( ".tc-prop" ).prop( "disabled", true );
}

/* изменение свойств т.-клиента */
$tc_prop.on( "focusout", function () {
	var cm_old = $list_tcs.find( "option[value=" + tc_change + "]" ).text();
	...
	$list_tcs.find( "option[value=" + tc_change + "]" )
		.attr( "data", cm + "|" + cl + "|" + ht + "|" + pt + "|" + lg + "|" + pw )
		.text( cm );
	$( "#list-ss" ).find( "option" ).each( function () {
...
$( "#del-tc" ).on( "click", function () {
	if ( $list_tcs.val() ) {
		i = $list_tcs.find( ":selected" ).index();
		$list_tcs.find( ":selected" ).remove();
		q = $list_tcs.find( "option" ).length;
		if ( q == 1 ) {
			$tc_prop.find( ".tc-prop" ).val( '' ).prop( "disabled", true );
		} else {
			var i = q == i ? i : ++i;
			$list_tcs.find( ":nth-child(" + i + ")" ).prop( "selected", "selected" ).change();
		}
	}
	$( "#list-ss" ).find( "option" ).each( function () {
		var data = $( this ).attr( "data" );
		var cl = data.split( "|" );
		var value = $list_tcs.find( "option" ).filter( function () {
...
$( "#add-tc" ).on( "click", function () {
	$tc_prop.find( ".tc-prop" ).prop( "disabled", false );
	...
	if ( $list_tcs.val() ) {
		var num_new = 0;
		$list_tcs.find( "option" ).each( function () {
		...
		$list_tcs.append( '<option value="' + q + '" data="' + cm_new + '|' + cl +
			'|' + ht + '|' + pt + '|' + lg + '|' + pw + '">' + cm_new + '</option>' );
	} else {
		$list_tcs.append( '<option value="' + q + '" data="client1|utorrent||||">client1</option>' );
	}
	$list_tcs.find( "option[value=" + q + "]" ).prop( "selected", "selected" ).change();
	doSortSelect( "list-tcs" );
} );
...
$( "#online-tc" ).on( "click", function () {
	if ( $list_tcs.val() ) {
		var data = $list_tcs.find( ":selected" ).attr( "data" );
		data = data.split( "|" );
		$.ajax( {
...
function listClientsRefresh() {
	$( "#ss-client" ).find( "option" ).each( function () {
		if ( $( this ).val() != 0 ) {
			$( this ).remove();
		}
	} );
...
$tc_prop.focusout( listClientsRefresh );
