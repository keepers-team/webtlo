    ...
    if ($forumReportAvailable && isset($reports)) {
        ...
        if (count($messages) > count($postList)) {
            ...
            for ($i = 1; $i <= $count_post_reply; $i++) {
                $log->debug("Вставка дополнительного $i-ого сообщения...");
        ...
        foreach ($postList as $index => $postId) {
            $post_number = $index + 1;
            $log->debug("Редактирование сообщения № $post_number...");
...
if ($sendViaApi > 0) {
    $log->info(sprintf('Отчётов отправлено через API %d шт.', $sendViaApi));
}

if (count($editedTopicsIDs)) {
    $log->info(sprintf('Обработано подразделов: %d шт.', count($editedTopicsIDs)));
}
