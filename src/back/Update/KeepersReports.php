...
final class KeepersReports
{
     ...
     * @throws Exception
     */
    public function update(array $cfg): void
    {
        $user = $this->apiReport->apiCredentials($cfg);
        ...
        if (isset($cfg['subsections'])) {
            ...
            foreach ($cfg['subsections'] as $forumId => $subsection) {
                try {
                    $forumReports = $this->apiReport->getKeepers($forumId);
                } catch (Throwable $e) {
                ...
                foreach ($forumReports->keepers as $keeperReport) {
                    ...
                    if ($keeperReport->keeperId === $user->userId) {
                        continue;
                    }

                    /** @var KeeperData $keeper Данные о хранителе. */
