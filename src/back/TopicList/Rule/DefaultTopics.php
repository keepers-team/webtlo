...
final class DefaultTopics implements ListInterface
{
    ...
    private function queryTopics(
        ...
        Log::append(
            sprintf(
                'Topics: %d, KeepersTable: %d, KeepersList: %d',
                $this->db->selectRowsCount('DefaultRuleTopics'),
                $this->db->selectRowsCount('DefaultRuleKeepers'),
                count($this->keepers),
            )
        );
    ...
    private function getStatement(
        ...
        $statement = "
            SELECT
                ...
                Torrents.client_id,
                %s
            FROM DefaultRuleTopics AS Topics
            LEFT JOIN Torrents ON Topics.info_hash = Torrents.info_hash
            %s
            LEFT JOIN DefaultRuleKeepers AS Keepers
