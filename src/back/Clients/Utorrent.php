...
final class Utorrent implements ClientInterface
{
    ...
    public function getTorrents(array $filter = []): Torrents
    {
        ...
        $this->logger->debug('Start getting torrents');
        $generator = $this->generateTorrentsList(simpleRun: $simpleRun);

        $this->logger->debug('Got torrents generator. Start processing');
        $torrents = [];
    ...
    {
        $this->logger->debug('Start generator');
        ...
        $this->logger->debug('Done process');

        if (!$simpleRun) {
            $this->logger->debug('Start topic id fillers');
