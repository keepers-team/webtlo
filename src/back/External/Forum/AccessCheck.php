...
trait AccessCheck
{
    ...
    {
        // Сперва проверим, что доступ к форуму вообще есть.
        $availability = $this->checkAvailability();
        if (!$availability) {
            $this->logger->error('Не удалось подключиться к форуму. Проверьте настройки доступа в настройках.');

            return false;
        }

        // Теперь проверим статус авторизации.
