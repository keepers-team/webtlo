...
{
    public static function populate(TIniFileEx $ini, DB $db): array
    {
        $config = [];
        ...
        if (isset($config['clients']) && is_array($config['clients'])) {
            $config['clients'] = Utils::natsort_field($config['clients'], 'cm');
        }
        ...
        if (!empty($subsections)) {
            ...
            $config['subsections'] = Utils::natsort_field($config['subsections'], 'na');
        }
